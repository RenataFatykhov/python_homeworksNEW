import pytest
import requests
from db_config import BASE_URL
...
@pytest.fixture
def test_create_employee(headers, employee_data):
    response = requests.post(f"{BASE_URL}/employee", headers=headers, json=employee_data)
...
def test_patch_employee(headers, test_create_employee):
    employee_id = test_create_employee
    payload = {"position": "Senior QA Engineer"}
    response = requests.patch(f"{BASE_URL}/employee/{employee_id}", headers=headers, json=payload)
    assert response.status_code == 200, "Ошибка при обновлении сотрудника"
    assert response.json().get('position') == "Senior QA Engineer", "Позиция сотрудника не обновлена"
